...
class IPAdapter:
    ...
    def get_image_embeds(self, pil_image=None, clip_image_embeds=None):
        if pil_image is not None:
            ...
            clip_image_embeds = self.image_encoder(clip_image.to(self.device, dtype=torch.float16)).image_embeds #torch.Size([1, 1024])
        else:
    ...
    def generate(
        ...
        **kwargs,
        ...
        with torch.inference_mode():
            prompt_embeds_, negative_prompt_embeds_ = self.pipe.encode_prompt( #both torch.Size([4, 77, 768])
                prompt,
                device=self.device,
